...
const TIMEOUT = process.env.API_TIMEOUT_LIMIT ? Number.parseInt(process.env.API_TIMEOUT_LIMIT, 10) : 60000;
...
const commonMiddleware = (handler) => {
    ...
    const vercelHandler = async (request, response) => {
        ...
        try {
            ...
            const handlerResponse = await Promise.race([handler(url, request), createTimeoutPromise(TIMEOUT)]);

            if (handlerResponse.body && handlerResponse.statusCode) {
                response.status(handlerResponse.statusCode).json(handlerResponse.body);
            } else {
                response.status(200).json(typeof handlerResponse === 'object' ? handlerResponse : JSON.parse(handlerResponse));
            }
        } catch (error) {
            let errorCode = 500;
    ...
    const netlifyHandler = async (event, context, callback) => {
        ...
        try {
            // Race the handler against the timeout
            const handlerResponse = await Promise.race([handler(url, event, context), createTimeoutPromise(TIMEOUT)]);
